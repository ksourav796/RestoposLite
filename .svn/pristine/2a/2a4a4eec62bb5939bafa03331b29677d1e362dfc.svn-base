...
app.controller('onlineInvoicecontroller', function ($scope, $http, $rootScope, Notification) {
    ...
    $scope.onPageLoadData = function (searchText) {
        ...
        $http.get("/reports/sales/onlineInvoice", {
            params: {
    ...
    $scope.getPaginatedSalesInvoice = function (page) {
        ...
        if (!$scope.isFilterApplied) {
            ...
            $http.get("/reports/restaurantInvoicePaymentList?type="+"onlineinvoice",{
                params: {
                    "fromDate": $scope.fromDate,
                    "toDate": $scope.toDate,
