...
app.controller('waiterreportcontroller', function ($scope, $http, $rootScope, Notification) {
    ...
    $scope.onPageLoadData = function (searchText) {
        $scope.invoiceTotal = 0;
        $scope.receivedTotal = 0;
        $http.get("/reports/sales/restaurantInvoice", {
            params: {
                "fromDate": $scope.fromdate,
                "toDate": $scope.toDate,
    ...
    $scope.totalValues = function (val) {
        angular.forEach(val, function (value, key) {
            ...
            $scope.incentives = $scope.incentives + value.incentives;
            $scope.incentivesAmount = $scope.incentivesAmount + value.incentivesAmount;
